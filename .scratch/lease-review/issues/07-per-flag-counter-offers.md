...
2026-09-22: added a second model call (ADR 0013) that checks each flag's counter-offer against its own residual-risk statement before the review is returned. A residual-risk claim the check cannot confirm is replaced with an honest placeholder; the counter-offer stays visible and the flag is never withheld over this. Strengthened the residual-risk prompt instruction to require rereading the counter-offer before writing it. Full suite (220 tests), typecheck, lint, offline smoke, fixture build, and normal build all pass. This addresses the *mechanism* the live finding above exposed — a check that can catch a paraphrased contradiction, not just a verbatim one — but it is a second model's judgment, not independent adjudication; the plausibility criterion and ticket 09's evidence gate remain open.
