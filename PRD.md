...
2. **Take a complete residential lease packet.** Require the signer to be signed in before they paste or upload anything, so every piece of extracted text has an owner from the start. Accept pasted text and PDFs with selectable text, including every document the lease references. Parse files in the browser and store only extracted text. If any referenced document is missing, name what is missing and stop before producing even a partial review.
